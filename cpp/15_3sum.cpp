      ...
      for (auto iter_i = set.begin(); iter_i != set.end(); ++iter_i) {
        ...
        for (auto iter_j = iter_i; iter_j != set.end(); ++iter_j) {
          if(iter_j->second[0] != 0){
            ...
            int sum = iter_i->first + iter_j->first;
            auto it = set.find(-sum);
            if(it != set.end() && it->second[1] >= iter_j->second[1] && it->second[0]!=0) {
              int temp[] = {iter_i->first, iter_j->first, -sum};
              res.emplace_back(temp, temp+3);
            }
